...
{
    semctl(semid, 0, IPC_RMID, NULL);// semafor deist
}
...
void* mapSharedMemory(int shmId)
{
    ...
    memoryAddress = shmat(shmId, NULL, 0);// возвращает адрес присоединенного сегмента разделяемой 
...
int main()
{
    ...
    semaphoreKey = ftok(constants::kInitialPath, constants::kKeyIdSem);//которая генерирует IPC ключ на основе пути к какому-то обязательно
    ...
    shMemoryId = shmget(shMemoryKey, constants::kSize, IPC_CREAT | SHM_R | SHM_W);//Сегмент разделяемой памяти
    if (shMemoryId == -1)
    {
        cerr << "Error: " << strerror(errno) << endl;
    ...
    case -1:
    {
        ...
        deleteSemaphoreSet(semaphoreId);
        shmdt(shMemoryAddress);//otsoedin
        shmctl(shMemoryId, IPC_RMID, &shMemoryStruct);
    ...
    default:
    {
        ...
        while(true)
        {
            ...
            {
                readyForInput = true;
                cout << "Press '0' to exit, or press any key" << endl;
